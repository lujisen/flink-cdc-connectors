...
public class PostgresChangeRecordEmitter
        extends RelationalChangeRecordEmitter<io.debezium.connector.highgo.PostgresPartition> {
    ...
    private final io.debezium.connector.highgo.PostgresConnectorConfig connectorConfig;
    ...
    public PostgresChangeRecordEmitter(
            io.debezium.connector.highgo.PostgresPartition partition,
    ...
    public void emitChangeRecords(
            ...
            Receiver<io.debezium.connector.highgo.PostgresPartition> receiver)
    ...
    @Override
    protected void emitTruncateRecord(Receiver receiver, TableSchema tableSchema)
            throws InterruptedException {
        Struct envelope =
                tableSchema
                        .getEnvelopeSchema()
        ...
        receiver.changeRecord(
                getPartition(), tableSchema, Operation.TRUNCATE, null, envelope, getOffset(), null);
    }
    ...
    private Object[] columnValues(
        ...
        Object[] values =
                new Object
                        [columnsWithoutToasted.size() < schemaColumns.size()
                                ? schemaColumns.size()
                                : columnsWithoutToasted.size()];

        final Set<String> undeliveredToastableColumns =
                new HashSet<>(schema.getToastableColumnsForTableId(table.id()));
        for (ReplicationMessage.Column column : columns) {
            // DBZ-298 Quoted column names will be sent like that in messages, but stored unquoted
            // in the column names
    ...
    private void refreshTableFromDatabase(TableId tableId) {
        try {
            schema.refresh(
                    connection, tableId, connectorConfig.skipRefreshSchemaOnMissingToastableData());
        } catch (SQLException e) {
    ...
    private boolean schemaChanged(
        ...
        if (msgHasMissingColumns && connectorConfig.skipRefreshSchemaOnMissingToastableData()) {
            // if we are ignoring missing toastable data for the purpose of schema sync, we need to
            // modify the
            // hasMissingColumns boolean to account for this. If there are untoasted columns missing
            // from the replication
            // message, we'll still have missing columns and thus require a schema refresh. However,
            // we can /possibly/
        ...
        if (msgHasMissingColumns || msgHasAdditionalColumns) {
            // the table metadata has less or more columns than the event, which means the table
        ...
        // go through the list of columns from the message to figure out if any of them are new or
        // have changed their type based
                        ...
                        message -> {
                            ...
                            if (column == null) {
                                ...
                                if (localType != incomingType) {
                                    final int incomingRootType =
                                            message.getType().getRootType().getOid();
                                ...
                                if (metadataInMessage) {
                                    ...
                                    final int incomingLength =
                                            message.getTypeMetadata().getLength();
    ...
    private boolean hasMissingUntoastedColumns(
        ...
        // Return `true` if we have some columns not in the replication message that are not
        // toastable or that we do
        // not recognize
    ...
    private Table tableFromFromMessage(List<ReplicationMessage.Column> columns, Table table) {
        final TableEditor combinedTable =
                table.edit()
                        .setColumns(
                                columns.stream()
                                        .map(
                                                column -> {
                                                    ...
                                                    // as long as default value is not added to the
                                                    // decoded message metadata, we must apply
