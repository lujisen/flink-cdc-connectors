 ...
 * io.debezium.connector.highgo.PostgresConnectorConfig}.
 *
...
public class PostgresConnector extends RelationalBaseSourceConnector {
    ...
    public List<Map<String, String>> taskConfigs(int maxTasks) {
        ...
        return props == null
                ...
                : Collections.singletonList(new HashMap<>(props));
    }
    ...
    public ConfigDef config() {
        return io.debezium.connector.highgo.PostgresConnectorConfig.configDef();
    }

    @Override
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        final ConfigValue databaseValue =
                configValues.get(RelationalDatabaseConnectorConfig.DATABASE_NAME.name());
        final ConfigValue slotNameValue =
                configValues.get(
                        io.debezium.connector.highgo.PostgresConnectorConfig.SLOT_NAME.name());
        final ConfigValue pluginNameValue =
                configValues.get(
                        io.debezium.connector.highgo.PostgresConnectorConfig.PLUGIN_NAME.name());
        ...
        final io.debezium.connector.highgo.PostgresConnectorConfig postgresConfig =
        ...
        try (PostgresConnection connection =
                new PostgresConnection(
                        ...
                        PostgresConnection.CONNECTION_VALIDATE_CONNECTION)) {
            try {
                ...
                if (!connection.queryAndMap(
                        "SELECT r.rolcanlogin AS rolcanlogin, r.rolreplication AS rolreplication,"
                                +
                                // for AWS the user might not have directly the rolreplication
                                // rights, but can be assigned
                                // to one of those role groups: rds_superuser, rdsadmin or
                                // rdsrepladmin
