...
import io.debezium.connector.highgo.connection.LogicalDecodingMessage;
...
/**
 * The class receives {@link LogicalDecodingMessage} events and delivers the event to the dedicated
 * topic.
 *
 * <p>Every {@code MESSAGE} event has its {@code payload} block enriched to contain
 *
 * <ul>
 *   <li>boolean that signifies if the message is transactional
 *   <li>message prefix
 *   <li>message content that is converted based on the connector's configured {@code
...
public class LogicalDecodingMessageMonitor {
     ...
     *
     * <p>{@code { "prefix" : "my-prefix" } }
     *
     * <p>Using a struct over the plain prefix as a string for better evolvability down the road.
     */
    ...
    public LogicalDecodingMessageMonitor(
            PostgresConnectorConfig connectorConfig, BlockingConsumer<SourceRecord> sender) {
        ...
        this.keySchema =
                SchemaBuilder.struct()
                        .name(schemaNameAdjuster.adjust("io.debezium.connector.highgo.MessageKey"))
                        .field(
        ...
        this.blockSchema =
                SchemaBuilder.struct()
                        .name(schemaNameAdjuster.adjust("io.debezium.connector.highgo.Message"))
                        .field(
                                DEBEZIUM_LOGICAL_DECODING_MESSAGE_PREFIX_KEY,
                                Schema.OPTIONAL_STRING_SCHEMA)
                        .field(
                                DEBEZIUM_LOGICAL_DECODING_MESSAGE_CONTENT_KEY,
                                binaryMode.getSchema().optional().build())
                        .build();
                ...
                SchemaBuilder.struct()
                        .name(
                                schemaNameAdjuster.adjust(
                                        "io.debezium.connector.highgo.MessageValue"))
    ...
    public void logicalDecodingMessageEvent(
            Partition partition,
        ...
        logicalMsgStruct.put(
                DEBEZIUM_LOGICAL_DECODING_MESSAGE_CONTENT_KEY,
                convertContent(message.getContent()));
