...
public class PostgresSnapshotChangeEventSource
    ...
    public PostgresSnapshotChangeEventSource(
            PostgresConnectorConfig connectorConfig,
            Snapshotter snapshotter,
            PostgresConnection jdbcConnection,
            PostgresSchema schema,
            EventDispatcher<io.debezium.connector.highgo.PostgresPartition, TableId> dispatcher,
            Clock clock,
    ...
    protected SnapshottingTask getSnapshottingTask(
            ...
            io.debezium.connector.highgo.PostgresOffsetContext previousOffset) {
    ...
    protected SnapshotContext<
                    io.debezium.connector.highgo.PostgresPartition,
                    io.debezium.connector.highgo.PostgresOffsetContext>
            prepare(io.debezium.connector.highgo.PostgresPartition partition) throws Exception {
    ...
    protected Set<TableId> getAllTableIds(
        ...
        return jdbcConnection.readTableNames(
                ctx.catalogName, null, null, new String[] {"TABLE", "PARTITIONED TABLE"});
    }
    ...
    protected void determineSnapshotOffset(
            ...
            io.debezium.connector.highgo.PostgresOffsetContext previousOffset)
            throws Exception {
        io.debezium.connector.highgo.PostgresOffsetContext offset = ctx.offset;
        if (offset == null) {
            if (previousOffset != null && !snapshotter.shouldStreamEventsStartingFromSnapshot()) {
                ...
                offset =
                        io.debezium.connector.highgo.PostgresOffsetContext.initialContext(
                                connectorConfig,
                                jdbcConnection,
                                getClock(),
    ...
    private void updateOffsetForSnapshot(io.debezium.connector.highgo.PostgresOffsetContext offset)
            throws SQLException {
    ...
    protected void updateOffsetForPreSnapshotCatchUpStreaming(
            io.debezium.connector.highgo.PostgresOffsetContext offset) throws SQLException {
        updateOffsetForSnapshot(offset);
    ...
    protected void readTableStructure(
            ...
            RelationalSnapshotContext<
                            ...
                            io.debezium.connector.highgo.PostgresOffsetContext>
                    snapshotContext,
            io.debezium.connector.highgo.PostgresOffsetContext offsetContext)
            throws SQLException, InterruptedException {
        Set<String> schemas =
        ...
        for (String schema : schemas) {
            ...
            LOGGER.info(
                    "Reading structure of schema '{}' of catalog '{}'",
                    schema,
    ...
    private static class PostgresSnapshotContext
            extends RelationalSnapshotContext<
                    io.debezium.connector.highgo.PostgresPartition, PostgresOffsetContext> {

        public PostgresSnapshotContext(PostgresPartition partition, String catalogName)
                throws SQLException {
