...
import io.debezium.connector.highgo.connection.AbstractMessageDecoder;
...
import io.debezium.connector.highgo.connection.TransactionMessage;
import io.debezium.connector.highgo.connection.wal2json.DateTimeFormat;
...
public class StreamingWal2JsonMessageDecoder extends AbstractMessageDecoder {
            ...
            if (!messageInProgress) {
                ...
                if (firstChar != LEFT_BRACE) {
                    ...
                    if (message.has("kind")) {
                        ...
                        currentChunk = null;
                        processor.process(
                                new TransactionMessage(Operation.BEGIN, txId, commitTime));
                    }
                }
            } else {
                nonInitialChunk(processor, typeRegistry, content);
    ...
    protected void nonInitialChunk(
            ReplicationMessageProcessor processor, TypeRegistry typeRegistry, final byte[] content)
