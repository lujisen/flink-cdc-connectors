...
import io.debezium.connector.highgo.connection.AbstractColumnValue;
...
public class PgProtoColumnValue extends AbstractColumnValue<PgProto.DatumMessage> {
    ...
    public Object asDecimal() {
        ...
        final String s = asString();
        if (s != null) {
            return PostgresValueConverter.toSpecialValue(s)
    ...
    public Instant asInstant() {
        if (value.hasDatumInt64()) {
            ...
            LOGGER.error(String.valueOf(value.getDatumInt64()));
        ...
        if(null!=s && s.startsWith("-")){
            LOGGER.info("PgProtoColumnValue()++++++:"+s);
            return s != null ? DateTimeFormat.get().timestampToInstant(asString().substring(1)) : null;
        }else{
            return s != null ? DateTimeFormat.get().timestampToInstant(asString()) : null;
        }
