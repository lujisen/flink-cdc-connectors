...
import io.debezium.connector.highgo.PostgresValueConverter;
...
import com.highgo.jdbc.jdbc.TimestampUtils;
...
public class PostgresDefaultValueConverter implements DefaultValueConverter {
    ...
    private Object convertDefaultValue(Object defaultValue, Column column) {
        ...
        if (valueConverters != null && defaultValue != null) {
            ...
            if ((result instanceof BigDecimal)
                ...
                result =
                        ((BigDecimal) result)
                                .setScale(column.scale().get(), RoundingMode.HALF_EVEN);
            }
            return result;
        }
        return defaultValue;
    ...
    private static Map<String, DefaultValueMapper> createDefaultValueMappers(
        ...
        result.put(
                ...
                (c, v) -> {
                    String defaultValue =
                            extractDefault(v, "00"); // if default is generated, assume length > 1
                ...
                parseNullDefault(
                        (c, v) ->
                                Boolean.parseBoolean(
                                        extractDefault(v, "false")))); // Sample value: true

        result.put("bpchar", (c, v) -> extractDefault(v, "")); // Sample value: 'abcd'::bpchar
        result.put(
                "varchar",
                (c, v) -> extractDefault(v, "")); // Sample value: `abcde'::character varying
        ...
        result.put(
                ...
                parseNullDefault(
                        (c, v) ->
                                new BigDecimal(
                                        extractDefault(v, "0.0")))); // Sample value: 12345.67891
        result.put(
                "float4",
                parseNullDefault(
                        (c, v) ->
                                Float.parseFloat(extractDefault(v, "0.0")))); // Sample value: 1.234
        result.put(
                "float8",
                parseNullDefault(
                        (c, v) ->
                                Double.parseDouble(
                                        extractDefault(v, "0.0")))); // Sample values: `1.234`,
        // `'12345678901234567890'::numeric`
        result.put(
                "int2",
                parseNullDefault(
                        (c, v) -> Short.parseShort(extractDefault(v, "0")))); // Sample value: 32767
        result.put(
                "int4",
                parseNullDefault(
                        (c, v) -> Integer.parseInt(extractDefault(v, "0")))); // Sample value: 123
        result.put("serial", parseNullDefault((c, v) -> Integer.parseInt(extractDefault(v, "0"))));
        result.put(
                "int8",
                parseNullDefault(
                        (c, v) -> Long.parseLong(extractDefault(v, "0")))); // Sample values: `123`,
        // `'9223372036854775807'::bigint`
        result.put("bigserial", parseNullDefault((c, v) -> Long.parseLong(extractDefault(v, "0"))));
        result.put(
                "smallserial",
                parseNullDefault((c, v) -> Short.parseShort(extractDefault(v, "0"))));
        ...
        result.put(
                ...
                (c, v) ->
                        UUID.fromString(
                                extractDefault(
                                        v,
                                        "00000000-0000-0000-0000-000000000000"))); // Sample value:
        // '76019d1a-ad2e-4b22-96e9-1a6d6543c818'::uuid

        result.put(
                "date", (c, v) -> timestampUtils.toLocalDateTime(extractDefault(v, "1970-01-01")));
        result.put("time", (c, v) -> timestampUtils.toLocalTime(extractDefault(v, "00:00")));
        result.put(
                "timestamp",
                (c, v) -> timestampUtils.toOffsetDateTime(extractDefault(v, "1970-01-01")));
        result.put(
                "timestamptz",
                (c, v) ->
                        timestampUtils
    ...
    private static String extractDefault(String defaultValue) {
        // Values are either "raw", such as `1234`, or "type casted", such as
        // `'9223372036854775807'::bigint`.
        // If the value does NOT contain a single quote it is assumed to be a raw value. Otherwise
        // the value is
