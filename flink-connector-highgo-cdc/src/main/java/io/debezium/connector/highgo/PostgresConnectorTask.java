...
import io.debezium.connector.highgo.connection.PostgresConnection.PostgresValueConverterBuilder;
import io.debezium.connector.highgo.connection.PostgresDefaultValueConverter;
...
/**
 * Kafka connect source task which uses Postgres logical decoding over a streaming replication
 * connection to process DB changes.
 *
 * @author Horia Chiorean (hchiorea@redhat.com)
 */
public class PostgresConnectorTask
        extends BaseSourceTask<
                PostgresPartition, io.debezium.connector.highgo.PostgresOffsetContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger(PostgresConnectorTask.class);
    private static final String CONTEXT_NAME = "highgo-connector-task";

    private volatile io.debezium.connector.highgo.PostgresTaskContext taskContext;
    ...
    private volatile io.debezium.connector.highgo.PostgresSchema schema;

    @Override
    public ChangeEventSourceCoordinator<
                    PostgresPartition, io.debezium.connector.highgo.PostgresOffsetContext>
            start(Configuration config) {
        final io.debezium.connector.highgo.PostgresConnectorConfig connectorConfig =
                new io.debezium.connector.highgo.PostgresConnectorConfig(config);
        ...
        this.taskContext =
                new io.debezium.connector.highgo.PostgresTaskContext(
                        connectorConfig, schema, topicSelector);
        final Offsets<PostgresPartition, io.debezium.connector.highgo.PostgresOffsetContext>
                previousOffsets =
                        getPreviousOffsets(
                                new PostgresPartition.Provider(connectorConfig),
                                new io.debezium.connector.highgo.PostgresOffsetContext.Loader(
                                        connectorConfig));
        final Clock clock = Clock.system();
        final io.debezium.connector.highgo.PostgresOffsetContext previousOffset =
                previousOffsets.getTheOnlyOffset();
        ...
        try {
            ...
            final io.debezium.connector.highgo.PostgresEventMetadataProvider metadataProvider =
                    new io.debezium.connector.highgo.PostgresEventMetadataProvider();

            final PostgresEventDispatcher<TableId> dispatcher =
    ...
    public ReplicationConnection createReplicationConnection(
            io.debezium.connector.highgo.PostgresTaskContext taskContext,
            boolean doSnapshot,
            int maxRetries,
            Duration retryDelay)
