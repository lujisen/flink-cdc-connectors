...
/**
 * Component that records the schema information for the {@link
 * io.debezium.connector.highgo.PostgresConnector}. The schema information contains the {@link
 * Tables table definitions} and the Kafka Connect {@link #schemaFor(TableId) Schema}s for each
 * table, where the {@link Schema} excludes any columns that have been {@link
 * io.debezium.connector.highgo.PostgresConnectorConfig#COLUMN_EXCLUDE_LIST specified} in the
 * configuration.
 *
...
public class PostgresSchema extends RelationalDatabaseSchema {
    ...
    /**
     * Create a schema component given the supplied {@link
     * io.debezium.connector.highgo.PostgresConnectorConfig Postgres connector configuration}.
     *
     * @param config the connector configuration, which is presumed to be valid
     */
    protected PostgresSchema(
            io.debezium.connector.highgo.PostgresConnectorConfig config,
