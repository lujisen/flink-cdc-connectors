...
import io.debezium.connector.highgo.PostgresConnectorConfig.HStoreHandlingMode;
import io.debezium.connector.highgo.PostgresConnectorConfig.IntervalHandlingMode;
import io.debezium.connector.highgo.data.Ltree;
...
import com.highgo.jdbc.util.HStoreConverter;
...
/**
 * A provider of {@link ValueConverter}s and {@link SchemaBuilder}s for various Postgres specific
 * column types.
 *
 * <p>In addition to handling data type conversion from values coming from JDBC, this is also
 * expected to handle data type conversion for data types coming from the logical decoding plugin.
 *
...
public class PostgresValueConverter extends JdbcValueConverters {
    ...
    public static final Instant POSITIVE_INFINITY_INSTANT =
            Conversions.toInstantFromMicros(PGStatement.DATE_POSITIVE_INFINITY);
    public static final LocalDateTime POSITIVE_INFINITY_LOCAL_DATE_TIME =
            LocalDateTime.ofInstant(POSITIVE_INFINITY_INSTANT, ZoneOffset.UTC);
    public static final OffsetDateTime POSITIVE_INFINITY_OFFSET_DATE_TIME =
            OffsetDateTime.ofInstant(
                    Conversions.toInstantFromMillis(PGStatement.DATE_POSITIVE_INFINITY),
                    ZoneOffset.UTC);
    ...
    public static final Instant NEGATIVE_INFINITY_INSTANT =
            Conversions.toInstantFromMicros(PGStatement.DATE_NEGATIVE_INFINITY);
    public static final LocalDateTime NEGATIVE_INFINITY_LOCAL_DATE_TIME =
            LocalDateTime.ofInstant(NEGATIVE_INFINITY_INSTANT, ZoneOffset.UTC);
    public static final OffsetDateTime NEGATIVE_INFINITY_OFFSET_DATE_TIME =
            OffsetDateTime.ofInstant(
                    Conversions.toInstantFromMillis(PGStatement.DATE_NEGATIVE_INFINITY),
                    ZoneOffset.UTC);
    ...
    /**
     * {@code true} if fields of data type not know should be handle as opaque binary; {@code false}
     * if they should be omitted
     */
    private final boolean includeUnknownDatatypes;

    private final io.debezium.connector.highgo.TypeRegistry typeRegistry;
    ...
    public static PostgresValueConverter of(
            PostgresConnectorConfig connectorConfig,
            Charset databaseCharset,
            io.debezium.connector.highgo.TypeRegistry typeRegistry) {
    ...
    protected PostgresValueConverter(
            Charset databaseCharset,
            ...
            IntervalHandlingMode intervalMode,
    ...
    public SchemaBuilder schemaBuilder(Column column) {
        ...
        switch (oidValue) {
            case io.debezium.connector.highgo.PgOid.BIT:
            case io.debezium.connector.highgo.PgOid.BIT_ARRAY:
            case io.debezium.connector.highgo.PgOid.VARBIT:
                return column.length() > 1 ? Bits.builder(column.length()) : SchemaBuilder.bool();
            case io.debezium.connector.highgo.PgOid.INTERVAL:
                return intervalMode == IntervalHandlingMode.STRING
                        ? Interval.builder()
                        : MicroDuration.builder();
            case io.debezium.connector.highgo.PgOid.TIMESTAMPTZ:
                // JDBC reports this as "timestamp" even though it's with tz, so we can't use the
                // base class...
                return ZonedTimestamp.builder();
            case io.debezium.connector.highgo.PgOid.TIMETZ:
                // JDBC reports this as "time" but this contains TZ information
                return ZonedTime.builder();
            case io.debezium.connector.highgo.PgOid.OID:
                return SchemaBuilder.int64();
            case io.debezium.connector.highgo.PgOid.JSONB_OID:
            case io.debezium.connector.highgo.PgOid.JSON:
                return Json.builder();
            case io.debezium.connector.highgo.PgOid.TSRANGE_OID:
            case io.debezium.connector.highgo.PgOid.TSTZRANGE_OID:
            case io.debezium.connector.highgo.PgOid.DATERANGE_OID:
            case io.debezium.connector.highgo.PgOid.INET_OID:
            case io.debezium.connector.highgo.PgOid.CIDR_OID:
            case io.debezium.connector.highgo.PgOid.MACADDR_OID:
            case io.debezium.connector.highgo.PgOid.MACADDR8_OID:
            case io.debezium.connector.highgo.PgOid.INT4RANGE_OID:
            case io.debezium.connector.highgo.PgOid.NUM_RANGE_OID:
            case io.debezium.connector.highgo.PgOid.INT8RANGE_OID:
                return SchemaBuilder.string();
            case io.debezium.connector.highgo.PgOid.UUID:
                return Uuid.builder();
            case io.debezium.connector.highgo.PgOid.POINT:
                return Point.builder();
            case io.debezium.connector.highgo.PgOid.MONEY:
                return moneySchema();
            case io.debezium.connector.highgo.PgOid.NUMERIC:
                return numericSchema(column);
            case io.debezium.connector.highgo.PgOid.BYTEA:
                return binaryMode.getSchema();
            case io.debezium.connector.highgo.PgOid.INT2_ARRAY:
                return SchemaBuilder.array(SchemaBuilder.OPTIONAL_INT16_SCHEMA);
            case io.debezium.connector.highgo.PgOid.INT4_ARRAY:
                return SchemaBuilder.array(SchemaBuilder.OPTIONAL_INT32_SCHEMA);
            case io.debezium.connector.highgo.PgOid.INT8_ARRAY:
                return SchemaBuilder.array(SchemaBuilder.OPTIONAL_INT64_SCHEMA);
            case io.debezium.connector.highgo.PgOid.CHAR_ARRAY:
            case io.debezium.connector.highgo.PgOid.VARCHAR_ARRAY:
            case io.debezium.connector.highgo.PgOid.TEXT_ARRAY:
            case io.debezium.connector.highgo.PgOid.BPCHAR_ARRAY:
            case io.debezium.connector.highgo.PgOid.INET_ARRAY:
            case io.debezium.connector.highgo.PgOid.CIDR_ARRAY:
            case io.debezium.connector.highgo.PgOid.MACADDR_ARRAY:
            case io.debezium.connector.highgo.PgOid.MACADDR8_ARRAY:
            case io.debezium.connector.highgo.PgOid.TSRANGE_ARRAY:
            case io.debezium.connector.highgo.PgOid.TSTZRANGE_ARRAY:
            case io.debezium.connector.highgo.PgOid.DATERANGE_ARRAY:
            case io.debezium.connector.highgo.PgOid.INT4RANGE_ARRAY:
            case io.debezium.connector.highgo.PgOid.NUM_RANGE_ARRAY:
            case io.debezium.connector.highgo.PgOid.INT8RANGE_ARRAY:
                return SchemaBuilder.array(SchemaBuilder.OPTIONAL_STRING_SCHEMA);
            case io.debezium.connector.highgo.PgOid.NUMERIC_ARRAY:
                return SchemaBuilder.array(numericSchema(column).optional().build());
            case io.debezium.connector.highgo.PgOid.FLOAT4_ARRAY:
                return SchemaBuilder.array(Schema.OPTIONAL_FLOAT32_SCHEMA);
            case io.debezium.connector.highgo.PgOid.FLOAT8_ARRAY:
                return SchemaBuilder.array(Schema.OPTIONAL_FLOAT64_SCHEMA);
            case io.debezium.connector.highgo.PgOid.BOOL_ARRAY:
                return SchemaBuilder.array(SchemaBuilder.OPTIONAL_BOOLEAN_SCHEMA);
            case io.debezium.connector.highgo.PgOid.DATE_ARRAY:
            ...
            case io.debezium.connector.highgo.PgOid.UUID_ARRAY:
                return SchemaBuilder.array(Uuid.builder().optional().build());
            case io.debezium.connector.highgo.PgOid.JSONB_ARRAY:
            case io.debezium.connector.highgo.PgOid.JSON_ARRAY:
                return SchemaBuilder.array(Json.builder().optional().build());
            case io.debezium.connector.highgo.PgOid.TIME_ARRAY:
                return SchemaBuilder.array(MicroTime.builder().optional().build());
            case io.debezium.connector.highgo.PgOid.TIMETZ_ARRAY:
                return SchemaBuilder.array(ZonedTime.builder().optional().build());
            case io.debezium.connector.highgo.PgOid.TIMESTAMP_ARRAY:
                if (adaptiveTimePrecisionMode || adaptiveTimeMicrosecondsPrecisionMode) {
                    if (getTimePrecision(column) <= 3) {
                        return SchemaBuilder.array(
                                io.debezium.time.Timestamp.builder().optional().build());
                    }
            ...
            case io.debezium.connector.highgo.PgOid.TIMESTAMPTZ_ARRAY:
                return SchemaBuilder.array(ZonedTimestamp.builder().optional().build());
            case io.debezium.connector.highgo.PgOid.OID_ARRAY:
                return SchemaBuilder.array(SchemaBuilder.OPTIONAL_INT64_SCHEMA);
            case io.debezium.connector.highgo.PgOid.BYTEA_ARRAY:
            case io.debezium.connector.highgo.PgOid.MONEY_ARRAY:
            case io.debezium.connector.highgo.PgOid.NAME_ARRAY:
            case io.debezium.connector.highgo.PgOid.INTERVAL_ARRAY:
            case io.debezium.connector.highgo.PgOid.VARBIT_ARRAY:
            case io.debezium.connector.highgo.PgOid.XML_ARRAY:
            case io.debezium.connector.highgo.PgOid.POINT_ARRAY:
            case io.debezium.connector.highgo.PgOid.REF_CURSOR_ARRAY:
                // These array types still need to be implemented. The superclass won't handle them
                // so
            ...
            default:
                ...
                final io.debezium.connector.highgo.PostgresType resolvedType =
                        typeRegistry.get(oidValue);

                if (resolvedType.isEnumType()) {
                    return io.debezium.data.Enum.builder(
                            Strings.join(",", resolvedType.getEnumValues()));
                    ...
                    if (resolvedType.getElementType().isEnumType()) {
                        ...
                        return SchemaBuilder.array(
                                io.debezium.data.Enum.builder(Strings.join(",", enumValues)));
                    } else {
                        // unfortunately, this does not work for array columns of domain types; the
                        // element type will have a
                        // non-matching JDBC id, resulting in no schema builder to be returned for
                        // those; the only way to export
                        ...
                        if (jdbcSchemaBuilder != null) {
                            ...
                            return includeUnknownDatatypes
                                    ? SchemaBuilder.array(binaryMode.getSchema())
                                    : null;
                        }
                    }
                } else {
    ...
    private SchemaBuilder numericSchema(Column column) {
        ...
        return SpecialValueDecimal.builder(
                decimalMode, column.length(), column.scale().orElseGet(() -> 0));
    }

    private SchemaBuilder hstoreSchema() {
        if (hStoreMode == HStoreHandlingMode.JSON) {
            return Json.builder();
        } else {
            // keys are not nullable, but values are
            return SchemaBuilder.map(
                    SchemaBuilder.STRING_SCHEMA, SchemaBuilder.OPTIONAL_STRING_SCHEMA);
        }
    }
    ...
    private SchemaBuilder arrayElementSchema(Column column) {
        io.debezium.connector.highgo.PostgresType arrayType = typeRegistry.get(column.nativeType());
        io.debezium.connector.highgo.PostgresType elementType = arrayType.getElementType();
        final String elementTypeName = elementType.getName();
        final String elementColumnName = column.name() + "-element";
        final Column elementColumn =
                Column.editor()
    ...
    public ValueConverter converter(Column column, Field fieldDefn) {
        ...
        switch (oidValue) {
            ...
            case io.debezium.connector.highgo.PgOid.TIME:
                return data -> convertTime(column, fieldDefn, data);
            case io.debezium.connector.highgo.PgOid.TIMESTAMP:
            ...
            case io.debezium.connector.highgo.PgOid.OID_ARRAY:
                ...
                // TODO DBZ-459 implement support for these array types; for now we just fall back
                // to the default, i.e.
    ...
    private ValueConverter createArrayConverter(Column column, Field fieldDefn) {
        ...
        Schema elementSchema = elementSchemaBuilder.optional().build();
    ...
    protected Object convertDecimal(Column column, Field fieldDefn, Object data, DecimalMode mode) {
        ...
        if (isVariableScaleDecimal(column) && mode == DecimalMode.PRECISE) {
            ...
            return VariableScaleDecimal.fromLogical(
                    fieldDefn.schema(), new SpecialValueDecimal(newDecimal));
        }

        return SpecialValueDecimal.fromLogical(
                new SpecialValueDecimal(newDecimal), mode, column.name());
    }

    protected Object convertHStore(
            Column column, Field fieldDefn, Object data, HStoreHandlingMode mode) {
     ...
     * i.e. the given byte array is NOT the byte representation returned by
    ...
    protected Object convertMoney(Column column, Field fieldDefn, Object data, DecimalMode mode) {
        return convertValue(
                ...
                fieldDefn,
                data,
                BigDecimal.ZERO.setScale(moneyFractionDigits),
                (r) -> {
                    switch (mode) {
                        case DOUBLE:
                        ...
                        case PRECISE:
                            if (data instanceof Double) {
                                r.deliver(
                                        BigDecimal.valueOf((Double) data)
                                                .setScale(
                                                        moneyFractionDigits, RoundingMode.HALF_UP));
                            } else if (data instanceof Number) {
                                // the plugin will return a 64bit signed integer where the last
                                // #moneyFractionDigits are always decimals
                                r.deliver(
                                        BigDecimal.valueOf(((Number) data).longValue())
                                                .setScale(
                                                        moneyFractionDigits, RoundingMode.HALF_UP));
                            }
                            break;
    ...
    protected Object convertTimestampWithZone(Column column, Field fieldDefn, Object data) {
        if (data instanceof Date) {
            // any Date like subclasses will be given to us by the JDBC driver, which uses the local
            // VM TZ, so we need to go
            // back to GMT
    ...
    protected Object convertTimeWithZone(Column column, Field fieldDefn, Object data) {
        ...
        if (data instanceof String) {
            ...
            final OffsetTime offsetTime =
                    OffsetTime.parse((String) data, TIME_WITH_TIMEZONE_FORMATTER);
    ...
    protected Object convertGeometry(Column column, Field fieldDefn, Object data) {
        final io.debezium.connector.highgo.PostgisGeometry empty =
                io.debezium.connector.highgo.PostgisGeometry.createEmpty();
        return convertValue(
                column,
                fieldDefn,
                data,
                ...
                (r) -> {
                    try {
                        ...
                        if (data instanceof byte[]) {
                            io.debezium.connector.highgo.PostgisGeometry geom =
                                    io.debezium.connector.highgo.PostgisGeometry.fromHexEwkb(
                                            new String((byte[]) data, "ASCII"));
    ...
    protected Object convertGeography(Column column, Field fieldDefn, Object data) {
        ...
        return convertValue(
                ...
                fieldDefn,
                data,
                Geography.createValue(fieldDefn.schema(), empty.getWkb(), empty.getSrid()),
                (r) -> {
                    final Schema schema = fieldDefn.schema();
                    try {
                        if (data instanceof byte[]) {
                            ...
                            io.debezium.connector.highgo.PostgisGeometry geom =
                                    PostgisGeometry.fromHexEwkb((String) data);
                            r.deliver(Geography.createValue(schema, geom.getWkb(), geom.getSrid()));
                        }
                    } catch (IllegalArgumentException | UnsupportedEncodingException e) {
     ...
     * @param data a data for the point column, either coming from the JDBC driver or logical
     *     decoding plugin
    ...
    protected Object convertPoint(Column column, Field fieldDefn, Object data) {
        return convertValue(
                ...
                fieldDefn,
                data,
                Point.createValue(fieldDefn.schema(), 0, 0),
                (r) -> {
                    final Schema schema = fieldDefn.schema();
                    if (data instanceof PGpoint) {
                        ...
                        try {
                            ...
                            logger.warn(
                                    "Error converting the string '{}' to a PGPoint type for the column '{}'",
                                    dataString,
                                    column);
                        }
                        ...
                        r.deliver(
                                Point.createValue(
                                        schema,
                                        ((PgProto.Point) data).getX(),
                                        ((PgProto.Point) data).getY()));
                    }
                });
    }

    protected Object convertArray(
            Column column,
            Field fieldDefn,
            io.debezium.connector.highgo.PostgresType elementType,
                            ...
                            for (Object value : values) {
                                converted.add(
                                        elementConverter.convert(
                                                resolveArrayValue(value, elementType)));
                            }
                            r.deliver(converted);
                        } catch (SQLException e) {
                            throw new ConnectException(
                                    "Failed to read value of array " + column.name());
                        }
                    }
                });
    }
                    ...
                    if (value instanceof java.sql.Time) {
                        ...
                        zonedDateTime =
                                zonedDateTime.plus(
                                        systemDefault()
                                                .getRules()
                                                .getDaylightSavings(zonedDateTime.toInstant()));
                        return OffsetTime.of(
                                zonedDateTime.withZoneSameInstant(ZoneOffset.UTC).toLocalTime(),
                                ZoneOffset.UTC);
                    }
                    break;
                }
            case PgOid.TIMESTAMPTZ:
                {
                    if (value instanceof Timestamp) {
                        ...
                        ZonedDateTime zonedDateTime =
                                ZonedDateTime.of(
                                        ((Timestamp) value).toLocalDateTime(), systemDefault());
    ...
    private boolean isVariableScaleDecimal(final Column column) {
        // TODO: Remove VARIABLE_SCALE_DECIMAL_LENGTH when
        // https://github.com/pgjdbc/pgjdbc/issues/2275
        // is closed.
    ...
    protected Object convertTimestampToLocalDateTime(Column column, Field fieldDefn, Object data) {
        ...
        final LocalDateTime utcTime = LocalDateTime.ofInstant(instant, ZoneOffset.systemDefault());
    ...
    protected Object convertBinaryToBytes(Column column, Field fieldDefn, Object data) {
        if (data
                == io.debezium.connector.highgo.UnchangedToastedReplicationMessageColumn
        ...
        return super.convertBinaryToBytes(
                column,
                fieldDefn,
    ...
    protected Object convertBinaryToBase64(Column column, Field fieldDefn, Object data) {
        return super.convertBinaryToBase64(
                column,
                fieldDefn,
                (data instanceof PGobject) ? ((PGobject) data).getValue() : data);
    }
    ...
    protected Object convertBinaryToHex(Column column, Field fieldDefn, Object data) {
        return super.convertBinaryToHex(
                column,
                fieldDefn,
                (data instanceof PGobject) ? ((PGobject) data).getValue() : data);
    }
