...
/**
 * The context of a {@link io.debezium.connector.highgo.PostgresConnectorTask}. This deals with most
 * of the brunt of reading various configuration options and creating other objects with these
 * various options.
 *
...
public class PostgresTaskContext extends CdcSourceTaskContext {
    ...
    private final io.debezium.connector.highgo.PostgresConnectorConfig config;
    ...
    protected io.debezium.connector.highgo.PostgresConnectorConfig config() {
        return config;
    }
    ...
    protected ReplicationConnection createReplicationConnection(
        ...
        if (dropSlotOnStop) {
            LOGGER.warn(
                    ...
                    io.debezium.connector.highgo.PostgresConnectorConfig.DROP_SLOT_ON_STOP.name());
        }
