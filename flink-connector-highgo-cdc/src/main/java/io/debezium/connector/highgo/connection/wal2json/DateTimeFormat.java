 ...
 */
package io.debezium.connector.highgo.connection.wal2json;
...
/**
 * Transformer for time/date related string representations in JSON messages coming from the
 * wal2json plugin.
 *
...
public interface DateTimeFormat {
    ...
    public static class ISODateTimeFormat implements DateTimeFormat {
        ...
        public Instant timestampToInstant(String s) {
           /*Modified  by  lujisen 2023-10-18------------start*/
            if(null!=s && s.startsWith("-")){
                return format(
                        TS_FORMAT_PATTERN_HINT,
                        s,
                        () -> LocalDateTime.from(TS_FORMAT.parse(s.substring(1))).toInstant(ZoneOffset.UTC));
            }else{
                return format(
                        TS_FORMAT_PATTERN_HINT,
                        s,
                        () -> LocalDateTime.from(TS_FORMAT.parse(s)).toInstant(ZoneOffset.UTC));
            }
            /*Modified  by  lujisen 2023-10-18------------end*/
        }
        ...
        public OffsetDateTime timestampWithTimeZoneToOffsetDateTime(String s) {
            return format(
                    ...
                    () -> {
                        ...
                        // The standard parsing library does not allow both optional minutes and
                        // seconds in offset,
                        // so it is necessary to parse it with optional minutes and if that fails
                        // then retyr with
                        // seconds
                        try {
        ...
        public Instant systemTimestampToInstant(String s) {
            return format(
                    ...
                    s,
                    () -> OffsetDateTime.from(SYSTEM_TS_FORMAT.parse(s)).toInstant());
        }
    }
}
