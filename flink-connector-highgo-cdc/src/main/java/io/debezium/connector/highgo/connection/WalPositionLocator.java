 ...
 * connector restarts. The LSNs are not guaranteed to be ordered in the WAL. LSN of commits are
 * ordered and LSNs inside a transaction are ordered. It is thus necessary to find out the beginning
 * of the unprocessed transaction and the LSN from which the streaming should start.
 *
...
public class WalPositionLocator {
    ...
    private final io.debezium.connector.highgo.connection.Lsn lastCommitStoredLsn;
    private final io.debezium.connector.highgo.connection.Lsn lastEventStoredLsn;
    private io.debezium.connector.highgo.connection.Lsn txStartLsn = null;
    private io.debezium.connector.highgo.connection.Lsn lsnAfterLastEventStoredLsn = null;
    private io.debezium.connector.highgo.connection.Lsn firstLsnReceived = null;
    private boolean passMessages = true;
    private io.debezium.connector.highgo.connection.Lsn startStreamingLsn = null;
    private boolean storeLsnAfterLastEventStoredLsn = false;
    private Set<io.debezium.connector.highgo.connection.Lsn> lsnSeen = new HashSet<>(1_000);

    public WalPositionLocator(
            io.debezium.connector.highgo.connection.Lsn lastCommitStoredLsn,
            io.debezium.connector.highgo.connection.Lsn lastEventStoredLsn) {
     ...
     *     not been found yet
     */
    public Optional<io.debezium.connector.highgo.connection.Lsn> resumeFromLsn(
            io.debezium.connector.highgo.connection.Lsn currentLsn, ReplicationMessage message) {
     ...
     */
    public boolean skipMessage(io.debezium.connector.highgo.connection.Lsn lsn) {
        if (passMessages) {
            return false;
        }
