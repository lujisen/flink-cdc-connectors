...
package com.ververica.cdc.connectors.highgo.source.fetch;
...
import com.ververica.cdc.connectors.highgo.source.offset.HighgoOffset;
import com.ververica.cdc.connectors.highgo.source.offset.HighgoOffsetUtils;
import com.ververica.cdc.connectors.highgo.source.utils.HighgoQueryUtils;
import io.debezium.config.Configuration;
import io.debezium.connector.highgo.PostgresConnectorConfig;
import io.debezium.connector.highgo.PostgresOffsetContext;
import io.debezium.connector.highgo.PostgresPartition;
...
import static io.debezium.connector.highgo.PostgresConnectorConfig.DROP_SLOT_ON_STOP;
import static io.debezium.connector.highgo.PostgresConnectorConfig.SLOT_NAME;
import static io.debezium.connector.highgo.PostgresObjectUtils.waitForReplicationSlotReady;
import static io.debezium.connector.highgo.Utils.currentOffset;
import static io.debezium.connector.highgo.Utils.refreshSchema;

/** A {@link FetchTask} implementation for Postgres to read snapshot split. */
public class HighgoScanFetchTask implements FetchTask<SourceSplitBase> {

    private static final Logger LOG = LoggerFactory.getLogger(HighgoScanFetchTask.class);

    private final SnapshotSplit split;
    private volatile boolean taskRunning = false;

    public HighgoScanFetchTask(SnapshotSplit split) {
        this.split = split;
    }
    ...
    public void execute(Context context) throws Exception {
        ...
        HighgoSourceFetchTaskContext ctx = (HighgoSourceFetchTaskContext) context;
        taskRunning = true;
        ...
        PostgresSnapshotSplitReadTask snapshotSplitReadTask =
                new PostgresSnapshotSplitReadTask(
                        ...
                        split,
                        ((HighgoSourceConfig) ctx.getSourceConfig()).getSlotNameForBackfillTask(),
                        ctx.getPluginName());
    ...
    private void executeBackfillTask(
            HighgoSourceFetchTaskContext ctx,
            SnapshotSplitChangeEventSourceContext changeEventSourceContext)
        ...
        final PostgresOffsetContext postgresOffsetContext =
                HighgoOffsetUtils.getPostgresOffsetContext(
                        loader, backfillSplit.getStartingOffset());

        // we should only capture events for the current table,
        // otherwise, we may not find corresponding schema
        HighgoSourceConfig config = (HighgoSourceConfig) ctx.getSourceConfig();
        ...
        final HighgoStreamFetchTask.StreamSplitReadTask backfillReadTask =
                new HighgoStreamFetchTask.StreamSplitReadTask(
    ...
    static class SnapshotSplitChangeEventSourceContext
        ...
        private HighgoOffset lowWatermark;
        private HighgoOffset highWatermark;

        public HighgoOffset getLowWatermark() {
            return lowWatermark;
        }

        public void setLowWatermark(HighgoOffset lowWatermark) {
            this.lowWatermark = lowWatermark;
        }

        public HighgoOffset getHighWatermark() {
            return highWatermark;
        }

        public void setHighWatermark(HighgoOffset highWatermark) {
    ...
    public static class PostgresSnapshotSplitReadTask
        ...
        protected SnapshotResult<PostgresOffsetContext> doExecute(
            ...
            final HighgoOffset lowWatermark = currentOffset(jdbcConnection);
            LOG.info(
            ...
            final HighgoOffset highWatermark = currentOffset(jdbcConnection);
            LOG.info(
        ...
        private void createDataEventsForTable(
            ...
            final String selectSql =
                    HighgoQueryUtils.buildSplitScanQuery(
            ...
            try (PreparedStatement selectStatement =
                            HighgoQueryUtils.readTableSplitDataStatement(
                                    jdbcConnection,
                                    selectSql,
                                    snapshotSplit.getSplitStart() == null,
                                    snapshotSplit.getSplitEnd() == null,
