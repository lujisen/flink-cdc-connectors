...
import com.ververica.cdc.connectors.highgo.HighgoSQLSource;
import com.ververica.cdc.connectors.highgo.source.HighgoSourceBuilder;
 ...
 * description.
 */
public class HighgoSQLTableSource implements ScanTableSource, SupportsReadingMetadata {
        ...
        DebeziumDeserializationSchema<RowData> deserializer =
                RowDataDebeziumDeserializeSchema.newBuilder()
                        ...
                        .setUserDefinedConverterFactory(
                                HighgoSQLDeserializationConverterFactory.instance())
                        .setValueValidator(new HighgoValueValidator(schemaName, tableName))
                        .setChangelogMode(changelogMode)
                        .build();

        if (enableParallelRead) {
            JdbcIncrementalSource<RowData> parallelSource =
                    HighgoSourceBuilder.PostgresIncrementalSource.<RowData>builder()
                            .hostname(hostname)
                            .port(port)
                            .database(database)
            ...
            DebeziumSourceFunction<RowData> sourceFunction =
                    HighgoSQLSource.<RowData>builder()
                            .hostname(hostname)
                            .port(port)
                            .database(database)
                            .schemaList(schemaName)
    ...
    private MetadataConverter[] getMetadataConverters() {
        ...
        return metadataKeys.stream()
                .map(
                        key ->
                                Stream.of(HighgoSQLReadableMetadata.values())
                                        .filter(m -> m.getKey().equals(key))
                                        .findFirst()
                                        .orElseThrow(IllegalStateException::new))
                .map(HighgoSQLReadableMetadata::getConverter)
                .toArray(MetadataConverter[]::new);
    }

    @Override
    public DynamicTableSource copy() {
        HighgoSQLTableSource source =
                new HighgoSQLTableSource(
                        physicalSchema,
                        port,
                        hostname,
                        database,
    ...
    public boolean equals(Object o) {
        ...
        HighgoSQLTableSource that = (HighgoSQLTableSource) o;
    ...
    public Map<String, DataType> listReadableMetadata() {
        return Stream.of(HighgoSQLReadableMetadata.values())
                .collect(
                        Collectors.toMap(
                                HighgoSQLReadableMetadata::getKey,
                                HighgoSQLReadableMetadata::getDataType));
    }
