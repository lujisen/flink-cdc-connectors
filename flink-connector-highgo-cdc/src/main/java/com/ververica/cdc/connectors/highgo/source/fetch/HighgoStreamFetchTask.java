...
import io.debezium.connector.highgo.PostgresStreamingChangeEventSource;
...
import static com.ververica.cdc.connectors.highgo.source.offset.HighgoOffset.NO_STOPPING_OFFSET;

/** A {@link FetchTask} implementation for Postgres to read streaming changes. */
public class HighgoStreamFetchTask implements FetchTask<SourceSplitBase> {
    private static final Logger LOG = LoggerFactory.getLogger(HighgoStreamFetchTask.class);
    ...
    public HighgoStreamFetchTask(StreamSplit streamSplit) {
    ...
    public void execute(Context context) throws Exception {
        ...
        HighgoSourceFetchTaskContext sourceFetchContext = (HighgoSourceFetchTaskContext) context;
        taskRunning = true;
        streamSplitReadTask =
    ...
    public static class StreamSplitReadTask extends PostgresStreamingChangeEventSource {
        ...
        public void execute(
            ...
            offsetContext.setStreamingStoppingLsn(
                    ((HighgoOffset) streamSplit.getEndingOffset()).getLsn());
            super.execute(context, partition, offsetContext);
            if (isBoundedRead()) {
                LOG.debug("StreamSplit is bounded read: {}", streamSplit);
                final HighgoOffset currentOffset = HighgoOffset.of(offsetContext.getOffset());
                try {
                ...
                ((HighgoScanFetchTask.PostgresChangeEventSourceContext) context).finished();
            }
        }
        ...
        private boolean isBoundedRead() {
            return !NO_STOPPING_OFFSET
                    .getLsn()
                    .equals(((HighgoOffset) streamSplit.getEndingOffset()).getLsn());
        }
    }
}
